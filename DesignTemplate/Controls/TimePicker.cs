...
namespace DesignTemplate.Controls
{
    /// <summary>
    ...
    public class TimePicker : Control
    {
        ...
        public static readonly DependencyProperty MinTimeProperty =
            DependencyProperty.Register("MinTime", typeof(TimeSpan), typeof(TimePicker), new UIPropertyMetadata(TimeSpan.MinValue,
                delegate(DependencyObject sender, DependencyPropertyChangedEventArgs e)
                {
                    ...
                    picker.MillisecondMinValue = picker.MinTime.Milliseconds;
        ...
        public static readonly DependencyProperty MaxTimeProperty =
            DependencyProperty.Register("MaxTime", typeof(TimeSpan), typeof(TimePicker), new UIPropertyMetadata(TimeSpan.MaxValue,
                delegate(DependencyObject sender, DependencyPropertyChangedEventArgs e)
                {
                    ...
                    picker.MillisecondMaxValue = picker.MaxTime.Milliseconds;
                    picker.CoerceValue(SelectedTimeProperty);//make sure to update the time if appropiate
                }));
            ...
            DependencyPropertyChangedEventArgs e)
        {
            ...
            if (!timePicker.isUpdatingTime)
            {
                ...
                if (timePicker.SelectedMillisecond != newTime.Milliseconds)
        ...
        /// <summary>
        /// Gets or sets the selected Millisecond
        /// </summary>
        public int SelectedMillisecond
        {
            get { return (int)GetValue(SelectedMillisecondProperty); }
            set { SetValue(SelectedMillisecondProperty, value); }
        }

        /// <summary>
        /// Backing store for the selected Millisecond
        /// </summary>
        public static readonly DependencyProperty SelectedMillisecondProperty =
            DependencyProperty.Register("SelectedMillisecond", typeof(int), typeof(TimePicker), new UIPropertyMetadata(0,
                delegate(DependencyObject sender, DependencyPropertyChangedEventArgs e)
                {
                    TimePicker timePicker = (TimePicker)sender;

                    //validate the minute set
                    int millisec = MathUtil.ValidateNumber(timePicker.SelectedMillisecond,
                        timePicker.MillisecondMinValue, timePicker.MillisecondMaxValue);
                    if (millisec != timePicker.SelectedMillisecond)
                        timePicker.SelectedMillisecond = millisec;
        ...
        public override void OnApplyTemplate()
        {
            ...
            //get the Milliseconds textbox and hook the events to it
            milliseconds = GetTemplateChild("PART_Milliseconds") as TextBox;
            milliseconds.PreviewTextInput += MillisecondsTextChanged;
            milliseconds.KeyUp += MillisecondsKeyUp;
            milliseconds.GotFocus += TextGotFocus;
            milliseconds.GotMouseCapture += TextGotFocus;
        ...
        //event handler for the textboxes (hours, minutes, seconds, milliseconds)
        private void TextGotFocus(object sender, RoutedEventArgs e)
        {
            ...
            //This field is used to check which entity(hour/minute/second/millisedonds) to increment/decrement when user clicks the buttuns
        ...
        private void SecondsTextChanged(object sender, TextCompositionEventArgs e)
        {
            ...
            //moves the carrot index or focus the neighbour
            AdjustCarretIndexOrMoveToNeighbour(seconds, milliseconds);

            //handle the event so that it does not set the text, since we do it manually
            e.Handled = true;
        }

        //event handler for the Millisecond TextBox
        private void MillisecondsTextChanged(object sender, TextCompositionEventArgs e)
        {
            //delete the text that is highlight(selected)
            TrimSelectedText(milliseconds);

            //Adjust the text according to the carrot index
            string newText = AdjustTextForMillisecond(milliseconds, e.Text);

            //validates that the millisecond is correct if not set a valid value (0 or 59)
            int secNum = ValidateAndSetMilliseconds(newText);

            //moves the carrot index or focus the neighbour
            AdjustCarretIndexOrMoveToNeighbour(milliseconds, null);
        ...
        {
            //focus the next control
            TryFocusNeighbourControl(seconds, minutes, milliseconds, e.Key);

            if (!IncrementDecrementTime(e.Key))
                ValidateAndSetSeconds(seconds.Text);
        }

        private void MillisecondsKeyUp(object sender, KeyEventArgs e)
        {
            //focus the next control
            TryFocusNeighbourControl(milliseconds, seconds, null, e.Key);

            if (!IncrementDecrementTime(e.Key))
                ValidateAndSetMilliseconds(milliseconds.Text);
        }
        #endregion
        ...
        //increment or decrement the time (hour/minute/second/millisencond) currently in selected (determined by the currentlySelectedTextBox that is set in the GotFocus event of the textboxes)
        private void IncrementDecrementTime(bool increment)
        {
            ...
            //check if senconds is selected if yes set it
            ...
            //if non of the above are selected assume that the milliseconds is selected
            else
                SelectedMillisecond = MathUtil.IncrementDecrementNumber(milliseconds.Text, MillisecondMinValue, MillisecondMaxValue, increment);
        }
        ...
        private int ValidateAndSetMilliseconds(string text)
        {
            int millisecNum = MathUtil.ValidateNumber(text, MillisecondMinValue, MillisecondMaxValue);
            SelectedMillisecond = millisecNum;
            return millisecNum;
        }
        #endregion
        ...
        //remove the left hand side number if the carrot index is 0 if the carrot index is 1 it removes the right hand side text
        private static string AdjustTextForMillisecond(TextBox textBox, string newText)
        {
            //replace the new text with the old text if there are already 3 char in the textbox For Millisecond
        ...
        //sets the selectedTime with the selectedhour, selectedminute, selectedsecond, selectedmillisecond
